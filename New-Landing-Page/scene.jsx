// scene.jsx — Hong Kong harbour scene components for DEVIEW hero video.
// Loaded after animations.jsx; exposes components to window for app.jsx.
...
function HoloPanel({ x, y, width, height, children, enter, exit, total }) {
  const time = useTime();
  let opacity = 0;
  let translate = 0;
  if (time < enter) {
...
function HoloOverlays({ duration }) {
  const time = useTime();
  return (
    <>
...
Object.assign(window, {
  Sky, Stars, AtmosphericHaze,
  FarSkyline, MidSkyline, Harbour,
  CameraDolly, HoloOverlays, ConnectionGraph,
  BrandReveal, Vignette, FilmGrain, LensGlints,
});
