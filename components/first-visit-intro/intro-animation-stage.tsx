// @ts-nocheck — ported animation timeline (New-Landing-Page)
"use client";
...
export type StageProps = {
  ...
  autoplay?: boolean;
  persistKey?: string;
  /** When false, playhead is not read/written to localStorage (first-visit intro). */
...
function Stage({
  ...
  useEffect(() => {
    if (!persistPlayhead) return;
    try {
      localStorage.setItem(persistKey + ":t", String(time));
    } catch {
      /* ignore */
    }
  }, [time, persistKey, persistPlayhead]);

  const barH = showPlaybackBar ? 44 : 0;
